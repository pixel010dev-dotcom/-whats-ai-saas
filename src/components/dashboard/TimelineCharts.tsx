...
function CustomTooltip({ active, payload, label, type }: { active?: boolean; payload?: { value: number }[]; label?: string; type: 'area' | 'bar' }) {
  ...
  return (
    <div className="bg-surface border border-theme rounded-lg px-3 py-2 shadow-xl">
...
function LoadingSkeleton() {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
      {[1, 2].map(i => (
        <div key={i} className="bg-surface border border-theme p-4 sm:p-5 rounded-xl">
          ...
          <div className="h-44 sm:h-48 bg-surface-hover/50 rounded animate-pulse" />
        </div>
      ))}
    </div>
  )
...
function EmptyState({ icon: Icon, title, desc }: { icon: React.ComponentType<{ className?: string }>; title: string; desc: string }) {
  return (
    <div className="bg-surface border border-theme p-4 sm:p-5 rounded-xl">
      <div className="flex items-center gap-3 mb-4">
        <Icon className="w-5 h-5 text-surface-hover" />
        <h3 className="text-sm font-medium text-secondary">{title}</h3>
      </div>
      <div className="h-44 sm:h-48 flex flex-col items-center justify-center text-center">
        <Icon className="w-10 h-10 text-surface-hover mb-2" />
      ...
      {hasConversations ? (
        <div className="bg-surface border border-theme p-4 sm:p-5 rounded-xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-sm font-medium text-primary">Conversas (30 dias)</h3>
          </div>
          <div className="h-44 sm:h-48">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data}>
                ...
                <YAxis tick={{ fontSize: 10, fill: 'var(--text-muted)' }} allowDecimals={false} axisLine={false} tickLine={false} width={24} />
      ...
      {hasRevenue ? (
        <div className="bg-surface border border-theme p-4 sm:p-5 rounded-xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-sm font-medium text-primary">Receita (30 dias)</h3>
          </div>
          <div className="h-44 sm:h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="date" tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickFormatter={(v: string) => v.slice(5)} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickFormatter={(v: number) => `R$${v}`} axisLine={false} tickLine={false} width={40} />
